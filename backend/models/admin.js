...
const getInfo = async () => {
  try {
    ...
    const q2 = `SELECT COUNT(order_id) AS totalOrders FROM \`order\``; // Escape "order" as it's a reserved keyword
    ...
    return {
      ...
      count: mostSaledItemResult[0]?.itemCount || 0, // Fix the typo and include itemCount
    };
  } catch (error) {
...
const abc = async () => {
  // Empty function for any future logic
};
